...
void loop() {
   ...
   while((millis()<(start_time+2085))&&(IRsensor_Front() < 1040))  //first task: look for IR, and spin around
      {
        motors(51,51,BACKWARD,BACKWARD,FORWARD,FORWARD);
      }
      Stop_Motor();
      delay(100);

      while((IRsensor_Front() > 1040) && (ultra_front() > 30)){ // found ir going forword
      ...
      while((IRsensor_Front() > 1020) && (ultra_front() < 30)){ // reach the ir stopping
      ...
      while((ultra_front() < 20) && (ultra_right() > 20)&&(ultra_left() > 20)){ // seeing obstical in front, rotet left antill ultra right see less then 40 
      ...
      while(ultra_left() < 30){ // obstical in left is wall
      ...
      delay(50);
        ...
        while(ultra_right() < 30){ // obstical in right is wall
